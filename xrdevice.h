...
using CallBack = void (Device::*)(const QByteArray&);
...
public:
    ...
    /// \return
    ///
    [[nodiscard]] static uint8_t calcCrc(const QByteArray& data);
    [[nodiscard]] static uint8_t calcCrc(const uint8_t* data, uint8_t len);

    Port* port() const;

protected:
    void ioRxPing(const QByteArray& data);
    void ioRxBufferOverflow(const QByteArray& data);
    void ioRxWrongCommand(const QByteArray& data);
    void ioRxCrcError(const QByteArray& data);
