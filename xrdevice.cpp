    ...
    , port_ { new Port(this) }
{
    ...
    port_->moveToThread(&portThread);
    connect(&portThread, &QThread::finished, port_, &QObject::deleteLater);
...
bool Device::ping(const QString& portName, int baud, [[maybe_unused]] int addr)
{
    ...
    do {
        if (port_->isOpen()) {
            emit close_();
            if (!semaphore_.tryAcquire(1, 2000)) {
